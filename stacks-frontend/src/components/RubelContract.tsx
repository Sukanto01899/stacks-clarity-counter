...
function RubelContract({ address }: { address: string }) {
  ...
  async function addMessage() {
    try {
      ...
      const result: TransactionResult = await request("stx_callContract", {
        contract: "SPFJRRM1NTW9V3AQ324NMWHSNAZQ6YM1D5XK1BHA.message-board",
  ...
  async function getMessageCountAtBlock() {
    try {
      const result = await fetchCallReadOnlyFunction({
        contractAddress: "SPFJRRM1NTW9V3AQ324NMWHSNAZQ6YM1D5XK1BHA",
  ...
  return (
    <main className="bg-amber-200/50 mt-4 w-1/3 mx-auto p-6 space-y-6">
      <div className="flex flex-col items-center justify-center">
        <h3 className="text-2xl font-semibold">Rubel Counter</h3>
