...
async function buildServer(serverOpts, chainhookOpts, predicates, callback) {
    async function waitForNode() {
        logger_1.logger.info(`ChainhookEventObserver looking for chainhook node at ${chainhookOpts.base_url}`);
        while (true) {
            try {
    ...
    async function isPredicateActive(predicate) {
        try {
            const result = await (0, undici_1.request)(`${chainhookOpts.base_url}/v1/chainhooks/${predicate.uuid}`, {
                method: 'GET',
            ...
            logger_1.logger.error(error, `ChainhookEventObserver unable to check if predicate ${predicate.uuid} is active`);
            return false;
        }
    }
    async function registerAllPredicates() {
        logger_1.logger.info(predicates, `ChainhookEventObserver connected to ${chainhookOpts.base_url}`);
            ...
            if (nodeType === 'chainhook') {
                switch (await isPredicateActive(predicate)) {
                    case undefined:
                        ...
                        break;
                    case true:
                        logger_1.logger.info(`ChainhookEventObserver predicate ${predicate.uuid} is already active, skipping registration`);
                        continue;
                    case false:
                        logger_1.logger.info(`ChainhookEventObserver predicate ${predicate.uuid} was being used but is now inactive, removing for re-regristration`);
                        await removePredicate(predicate);
                }
            }
            logger_1.logger.info(`ChainhookEventObserver registering predicate ${predicate.uuid}`);
            const thenThat = {
                http_post: {
    ...
    async function removePredicate(predicate) {
        const nodeType = serverOpts.node_type ?? 'chainhook';
        ...
        try {
            await (0, undici_1.request)(`${chainhookOpts.base_url}${path}`, {
                method: 'DELETE',
                headers: { 'content-type': 'application/json' },
                throwOnError: true,
            });
            logger_1.logger.info(`ChainhookEventObserver removed predicate ${predicate.uuid}`);
        }
        catch (error) {
            logger_1.logger.error(error, `ChainhookEventObserver unable to deregister predicate`);
        }
    }
    async function removeAllPredicates() {
        if (predicates.length === 0) {
    ...
    async function isEventAuthorized(request, reply) {
        if (!(serverOpts.validate_token_authorization ?? true))
            return;
